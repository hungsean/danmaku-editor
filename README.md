...
- 在播放器控制列加上一顆「彈幕工作台」按鈕，用來展開與收合工作台
- 在影片與標題選集區之間插入**時間軸工作台**，把你自己的彈幕草稿依時間排開
- 時間軸是跟隨播放的滾動視窗，播放頭固定在正中央，可縮放
- 三條軌道分別對應滑動、上方、下方，一眼看出類型；每則彈幕的寬度即其顯示時長
- 可拖曳彈幕區塊調整時間，也可點時間軸空白處跳轉影片
- 用目前播放位置快速打點（`使用目前時間` 按鈕與 `Enter` 逐句對歌詞）
- 新增、編輯、刪除彈幕草稿：時間、文字、位置、大小、顏色
- 從工作台控制頁面既有播放器的播放與暫停
- 在播放器上疊加本機預覽層，由影片時間觸發

時間軸上只會出現**你自己編輯的草稿**，動畫瘋原本的彈幕完全不會被讀取或顯示在這裡。
...
│  ├─ content/
│  │  ├─ main.ts           # 尋找 video、掛載兩個 Shadow DOM host、驅動每幀更新
│  │  ├─ timeline-panel.ts # 時間軸工作台（軌道、刻度、拖曳、詳細編輯列）
│  │  ├─ toggle-button.ts  # 偵測播放器控制列並注入切換按鈕
│  │  ├─ preview.ts        # 預覽層渲染
...
- 播放器控制列出現「彈幕工作台」按鈕，可展開與收合
- 影片與標題選集區之間出現時間軸工作台，標題選集被往下推而不是被蓋住
- 時間軸隨播放捲動，播放頭固定在正中央，時間文字更新（格式如 `01:23.4`）
- 工具列的播放／暫停可實際控制影片
- 初次載入有三則示範彈幕，分別落在滑動、上方、下方三條軌道上
- 拖曳彈幕區塊可改變時間；點時間軸空白處會跳轉影片
- 開啟預覽後彈幕會在對應時間疊到播放器上，三種位置有明顯不同的視覺效果
- 關閉預覽後所有自製彈幕立即消失
...
- **切換按鈕採自我驗證的兩段式安置**。先用啟發式找出「位於播放器底部、接近滿寬、高度矮、內含多個可點擊元素」的區塊當作控制列，依序嘗試三種插入點；每次插入後都會實際量測矩形，只要與控制列既有元素重疊就換下一種，全都重疊則完全撤出，改用疊在影片右上角的浮動按鈕。寧可按鈕不在控制列，也不蓋掉網站原本的功能。
  實測動畫瘋目前的播放器會走浮動退路，因為其控制列採絕對定位佈局，插入新元素不會被挪出空間
- **時間軸面板是插入頁面的節點**。它會卡在播放器容器與標題選集區之間，把下方內容往下推。若動畫瘋改版導致容器判定失準，面板可能出現在非預期的位置
